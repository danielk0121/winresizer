...
WinResizer가 창 위치를 조정하고 단축키를 감지하려면 macOS의 보안 권한이 필요합니다. 처음 실행 시 권한이 없다면 웹 UI 상단에 **"권한 필요"** 알림이 표시됩니다.

### 권한 부여 단계

1.  **손쉬운 사용 (Accessibility) 권한**:
    *   웹 UI의 [설정 창 열기] 버튼을 클릭하거나, **시스템 설정 > 개인정보 보호 및 보안 > 손쉬운 사용**으로 이동합니다.
    *   목록에서 `WinResizer`를 찾아 스위치를 **켬(On)**으로 변경합니다.
    *   **⚠️ 중요**: 권한을 허용하는 즉시 macOS 보안 정책에 의해 앱이 자동으로 종료될 수 있습니다. 이는 정상적인 동작입니다.
2.  **입력 모니터링 (Input Monitoring) 권한**:
    *   **시스템 설정 > 개인정보 보호 및 보안 > 입력 모니터링**으로 이동합니다.
    *   `WinResizer`가 목록에 있다면 허용해 주세요. (단축키 감지에 필요)
3.  **앱 재실행**:
    *   권한 설정 완료 후, 앱이 종료되었다면 다시 실행해 주세요.
    *   웹 UI 상단의 배지가 초록색 **"권한 승인됨"**으로 변경되면 모든 준비가 완료된 것입니다.

## 자주 묻는 질문 (FAQ)

**Q: 권한을 허용했는데 왜 앱이 갑자기 종료되나요?**
**A:** macOS는 보안을 위해 '손쉬운 사용' 권한이 변경된 앱을 강제로 종료시킵니다. 이는 시스템의 안전을 위한 표준 절차입니다. 권한 허용 후 앱을 다시 한 번 실행해 주시면 정상적으로 작동합니다.

**Q: 권한을 줬는데도 단축키가 작동하지 않아요.**
**A:** 가끔 macOS 설정이 즉시 반영되지 않을 수 있습니다. **시스템 설정 > 개인정보 보호 및 보안 > 손쉬운 사용**에서 `WinResizer`를 목록에서 제거(`-` 버튼)한 후, 앱을 다시 실행하여 권한을 다시 요청받거나 수동으로 추가해 보세요.

**Q: 웹 설정 페이지는 어떻게 들어가나요?**
**A:** 메뉴바 상단의 WinResizer 아이콘(트레이 아이콘)을 클릭한 후 **Preferences...** 메뉴를 선택하면 기본 브라우저에서 설정 페이지가 열립니다.

**Q: 특정 앱에서는 창 조절이 안 돼요.**
**A:** 일부 앱(예: 시스템 설정, 일부 보안 프로그램)은 macOS 보안 정책상 외부 앱의 창 제어를 허용하지 않을 수 있습니다. 그 외 일반적인 앱에서 작동하지 않는다면 권한 설정을 다시 확인해 주세요.
